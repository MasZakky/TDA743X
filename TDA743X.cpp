...
int8_t TDA7430::Mute(int8_t val,int8_t val2){
  ...
  if(val == 0){
    switch(val2){
      case ATT_LR: return TDA7430::Balance(_DATA[5],val2); break;
      case ATT_RR: return TDA7430::Balance(_DATA[6],val2); break;
      case ATT_LF: return TDA7430::Balance(_DATA[7],val2); break;
      case ATT_RF: return TDA7430::Balance(_DATA[8],val2); break;
    }
  }
  else{
    switch(val2){
...
int8_t TDA7432::Volume(int8_t val){
  ...
  if((_DATA[1]&0x7F) == val) return Process_OK;
  _DATA[1] = (1<<7) | val;
  return TDA7432::setWrite(1,_DATA[1]); 
}
int8_t TDA7432::Bass(int8_t val){
  val = constrain(val,-9,9);
  if(val <-7){ val = val * -1 - 7; _EBass = 0;}
  else if(val < 0){ val = val + 7; _EBass = 1;}
  else if(val > 7){ val = val - 1; _EBass = 0;}
  else {      val = 8 + (7 - val); _EBass = 1;}
  if(((_DATA[2]>>4) == val)&&(((_DATA[0]>>4)&0x01) == _EBass)) return Process_OK;
  _DATA[2] = (_DATA[4]&(~(0x0F<<4))) | (val << 4);
  _DATA[0] = (_DATA[0]&(~(1<<4))) | (_EBass << 4);
  TDA7432::setWrite(0,_DATA[0]); 
  return TDA7432::setWrite(2,_DATA[2]); 
}
int8_t TDA7432::Treble(int8_t val){
  val = constrain(val,-7,7);
  if(val < 0)  val = val + 7;
  else val = 8 + (7 - val);
...
int8_t TDA7432::Balance(int8_t val){
  TDA7432::Balance(val,ATT_LR);
  TDA7432::Balance(val,ATT_RR);
  TDA7432::Balance(val,ATT_LF);
  return TDA7432::Balance(val,ATT_RF);
}
int8_t TDA7432::Balance(int8_t val,int8_t val2){
  val = Constrain(val,0,79);
  val2 = constrain(val2,0,3);
  switch(val2){
    case ATT_LF: if(_DATA[3] != val)return TDA7432::setWrite(3,(_DATA[3]= val)); break;
    case ATT_RF: if(_DATA[4] != val)return TDA7432::setWrite(4,(_DATA[4]= val)); break; 
    case ATT_LR: if(_DATA[5] != val)return TDA7432::setWrite(5,(_DATA[5]= val)); break;
    case ATT_RR: if(_DATA[6] != val)return TDA7432::setWrite(6,(_DATA[6]= val)); break;
  }
  return Process_OK;
}

int8_t TDA7432::Input(int8_t val){
  val = constrain(val,1,2);
  if(val == 1) val = 0;
  if((_DATA[0]&0x03) == val) return Process_OK;
  _DATA[0] = (_DATA[9]&(~(0x03))) | val;
  return TDA7432::setWrite(0,_DATA[0]);
}

int8_t TDA7432::Mute(int8_t val){
  TDA7432::Mute(val,ATT_LR);
  TDA7432::Mute(val,ATT_RR);
  TDA7432::Mute(val,ATT_LF);
  return TDA7432::Mute(val,ATT_RF);
}
int8_t TDA7432::Mute(int8_t val,int8_t val2){
  val = constrain(val,0,1);
  val2 = constrain(val2,0,3);
  if(val == 0){
    switch(val2){
      case ATT_LR: return TDA7432::Balance(_DATA[3],val2); break;
      case ATT_RR: return TDA7432::Balance(_DATA[4],val2); break;
      case ATT_LF: return TDA7432::Balance(_DATA[5],val2); break;
      case ATT_RF: return TDA7432::Balance(_DATA[6],val2); break;
    }
  }
  else{
    switch(val2){
      case ATT_LR: return TDA7432::setWrite(3,(1<<5)); break;
      case ATT_RR: return TDA7432::setWrite(4,(1<<5)); break;
      case ATT_LF: return TDA7432::setWrite(5,(1<<5)); break;
      case ATT_RF: return TDA7432::setWrite(6,(1<<5)); break;
    }
  }
}
int8_t TDA7432::DisableMute(){
  TDA7432::Mute(MUTE_OFF);
}
int8_t TDA7432::EnableMute(){
  TDA7432::Mute(MUTE_ON);
}
...
int8_t TDA7432::UseValue(int8_t val){
  _UPN = val;
  return Process_OK;
}
int16_t TDA7432::Constrain(int16_t val,int16_t val2,int16_t val3){
  if(_UPN != Use_POSITIVE){
    return constrain(val,(-1*val3),val2);
  }
  else{
